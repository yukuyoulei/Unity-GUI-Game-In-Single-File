...
public class UBlackJack : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		if (Input.GetMouseButton(0) && rawPos.HasValue)
		{
			SwipingCardsX = SwipeCardsX + Input.mousePosition.x - rawPos.Value.x;
			SwipingCardsY = SwipeCardsY + rawPos.Value.y - Input.mousePosition.y;
			DrawCard(SwipingCardsX, SwipingCardsY, 0);
		}
		if (Input.GetMouseButtonUp(0) && rawPos.HasValue)
		{
